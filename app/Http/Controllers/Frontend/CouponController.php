...
class CouponController extends Controller
{
    ...
    public function apply(Request $request)
    {
        ...
        if($coupon != null && $request->shop_id != null && $coupon->shop_id != $request->shop_id){
            flash(translate('The coupon is invalid for this shop.'))->error();
            return back();
        }

        if ($coupon == null || strtotime(date('d-m-Y')) < $coupon->start_date || strtotime(date('d-m-Y')) > $coupon->end_date) {
            flash(translate('The coupon is invalid.'))->error();
            return back();
        }

        if (CouponUsage::where('user_id', auth()->user()->id)->where('coupon_id', $coupon->id)->first() != null) {
            flash(translate("You've already used the coupon."))->error();
            return back();
        }
        ...
        if ($coupon->type == 'cart_base') {
            ...
            if ($cartPrice >= $min_buy) {
                ...
                flash(translate('Coupon code applied successfully'))->success();
                return back()->with('coupon_details', $data);
            } else {
                flash(translate("Minimum order total of") . ' ' . format_price($min_buy) . ' ' . translate("is required to use this coupon code"))->error();
                return back();
            }
        }
        elseif ($coupon->type == 'product_base') {
            ...
            $cartItemsIds = Cart::where('user_id', auth()->user()->id)->pluck('product_id')->toArray();
            ...
            if ($discountApplicable) {
               $data['coupon_details'] = [
                ...
                flash(translate("This coupon code is no applicable for your cart products."))->error();
                return back();
            }
        }
    }
