...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($selected_attribute_values) {
            $products->with('variation_combinations')
                ->whereHas('variation_combinations', function ($query) use ($selected_attribute_values) {
    ...
    {
        $data['product'] = filter_products(Product::query())
...
//            return new ProductSingleCollection($product);
    ...
    public function search(Request $request)
    {
        ...
        $data['all_products'] = new ProductCollection($products->paginate(20));
