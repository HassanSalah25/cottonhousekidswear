...
class OrderController extends Controller
{
    ...
    {
        return new OrderCollection(CombinedOrder::with(['user','orders.orderDetails.variation.product','orders.orderDetails.variation.combinations','orders.shop'])->where('user_id', auth()->user()->id)->latest()->paginate(12));
    }
    ...
    public function show($order_code)
    {
        ...
        if($order){
            if(auth()->user()->id == $order->user_id){
                ...
                flash(translate("This order is not your. You can't check details of this order"))->success();
                return back();
            }
        }else{

            flash(translate("No order found by this code"))->success();
            return back();
        }
    }
    ...
    {
        $request->billing_address_id = $request->billing_address_id ? $request->billing_address_id : $request->shipping_address_id;
        ...
        return redirect()->route('profile.dashboard',['tab' => 'v-pills-orders-tab']);
    }
