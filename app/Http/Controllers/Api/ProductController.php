...
class ProductController extends Controller
{
    ...
    public function best_selling_products(Request $request)
    {
        $limit = $request->limit;
        ...
        if($category_slug)
            $products->whereHas('categories', function ($q) use ($category_slug) {
                $q->where('categories.slug', $category_slug);
            });
        return new ProductCollection(filter_products($products)->latest()->limit($limit)->get());
    }
...
//                }
//            });
            $products->where('name', 'like', '%' . $search_keyword . '%')->orWhere('tags', 'like', '%' . $search_keyword . '%');
        }
...
//                $attributes = Attribute::with('attribute_values')->whereIn('id', $attribute_ids)->get();
        ...
        if ($selected_attribute_values) {
            $products->with('variation_combinations')->whereHas('variation_combinations', function ($query) use ($selected_attribute_values) {
    ...
    public function ajax_search($search_keyword){
        ...
        $keywords = array();
        $products = Product::where('published', 1)->where('tags', 'like', '%' . $search_keyword . '%')->get();
