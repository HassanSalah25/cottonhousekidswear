...
if (!function_exists('areActiveRoute')) {
    function areActiveRoute( $route, $output = "active")
    {
        if ( url()->current() == url($route)) return $output;
    }
}
...
if (!function_exists('is_product_dscounted')) {
    function is_product_dscounted($product)
    {
        $check = ($product->discount > 0 &&
        ( Carbon::today()->timestamp >=Carbon::parse($product->discount_start_date)->timestamp
            && Carbon::today()->timestamp <  Carbon::parse($product->discount_end_date)->timestamp));
        return $check;
    }
}
...
if (!function_exists('generate_variation_options')) {
    ...
    function generate_variation_options($options)
    {
        ...
        foreach ($attrbute_ids as $id => $values) {
            ...
            $data['id'] = $id;
            $data['name'] = Attribute::find($id)?->getTranslation('name') ?? Attribute::find($id)?->name;
            $data['values'] = $vals;
...
if (!function_exists('banner_array_generate')) {
    function banner_array_generate($images, $links, $contents, $for_api = true)
    {
        if (!$images) {
            return [];
        }
        ...
        foreach ($images as $key => $value) {
            ...
            $arr['content'] = $for_api ? json_decode($contents, true)[$key] : $links[$key];
...
function translate($key, $lang = null, $addslashes = false)
{
    ...
    $translation_locale =  Translation::where('lang', $lang)->pluck('lang_value', 'lang_key')->toArray();
