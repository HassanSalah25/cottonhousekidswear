...
<script>
    ...
    function indexCart() {
        ...
        const bottom = $('.bottom');
        cartContainer.empty();
        var preloader = $(`@include('frontend.partials.components.preloader')`);
        cartContainer.append(preloader);
        $.ajax({
            ...
            beforeSend: function () {
                // Show preloader before the request is sent
                preloader.show();
            },
                ...
                cart_items.forEach(function (item) {
                    var html_item = `
                            <li>
                                ...
                                <p class="quantity">${item['qty']}X - <span class="amount">${item['dicounted_price']}</span> - <span>${item['combinations'].map(combine => combine['value']).join(', ')}</span></p>
            ...
            complete: function () {
                // Hide preloader after the request is complete (success or error)
                preloader.hide();
            }
        });
